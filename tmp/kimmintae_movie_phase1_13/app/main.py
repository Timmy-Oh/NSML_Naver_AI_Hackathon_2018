...
def bind_model(model):
    ...
    def infer(model, raw_data, dataset, **kwargs):
            """
            ...
            clipped = np.array(np.round(pred), dtype=np.int)
            # DONOTCHANGE: They are reserved for nsml
...
#     def get_model(embedding_matrix, config):
    def get_model(config):
        ...
        x1 = Embedding(config.max_features, config.embed_size, trainable = emb_train)(inp)
...
#         mdl = get_model(dataset.emb_matrix, config)
        ...
        hist = mdl.fit(dataset.reviews, dataset.labels, batch_size=config.batch_size, epochs=config.epochs, callbacks = [nsml_callback], verbose=2)
