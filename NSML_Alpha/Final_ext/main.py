...
def bind_model(model, config):
    ...
    def infer(raw_data, **kwargs):
        ...
        point = model.predict([preprocessed_data_pre, preprocessed_data_post])[-1]
        point[point>10.] = 10.
        point[point<1.] = 1.
...
#         inp = Input(shape=(config.max_features, ), name='input')
        ...
        embp = Embedding(config.max_features, config.max_features,embeddings_initializer='identity', trainable = True)(inp_post)
#         emb1 = Embedding(config.max_features, config.embed_size, trainable = True)(inp)
        embp1 = SpatialDropout1D(config.prob_dropout)(embp)
        
        #### 
        l1_Lp = Bidirectional(CuDNNLSTM(config.cell_size_l1, return_sequences=True))(embp1)
        
        l2_LLp = Bidirectional(CuDNNLSTM(config.cell_size_l2, return_sequences=True))(l1_Lp)
        l2_LGp = Bidirectional(CuDNNGRU(config.cell_size_l2, return_sequences=True))(l1_Lp)
        
        l3_LLCp = Conv1D(config.filter_size, kernel_size = config.kernel_size, strides=2, padding = "valid", kernel_initializer = "he_uniform")(l2_LLp)
        l3_LGCp = Conv1D(config.filter_size, kernel_size = config.kernel_size, strides=2, padding = "valid", kernel_initializer = "he_uniform")(l2_LGp)
        ...
        conc_LLCp = concatenate([avg_pool_Lp, max_pool_Lp, avg_pool_LLp, max_pool_LLp, avg_pool_LLCp, max_pool_LLCp, attention_LLAp, attention_LLCAp])
        conc_LGCp = concatenate([avg_pool_Lp, max_pool_Lp, avg_pool_LGp, max_pool_LGp, avg_pool_LGCp, max_pool_LGCp, attention_LGAp, attention_LGCAp])        
...
#         emb1 = Embedding(config.max_features, config.embed_size, trainable = True)(inp)
        ...
        #### 
        l1_Gp = Bidirectional(CuDNNGRU(config.cell_size_l1, return_sequences=True))(embp2)
        
        l2_GLp = Bidirectional(CuDNNLSTM(config.cell_size_l2, return_sequences=True))(l1_Gp)
        l2_GGp = Bidirectional(CuDNNGRU(config.cell_size_l2, return_sequences=True))(l1_Gp)
        
        l3_GLCp = Conv1D(config.filter_size, kernel_size = config.kernel_size, strides=2, padding = "valid", kernel_initializer = "he_uniform")(l2_GLp)
        l3_GGCp = Conv1D(config.filter_size, kernel_size = config.kernel_size, strides=2, padding = "valid", kernel_initializer = "he_uniform")(l2_GGp)
        ...
        conc_GLCp = concatenate([avg_pool_Gp, max_pool_Gp, avg_pool_GLp, max_pool_GLp, avg_pool_GLCp, max_pool_GLCp, attention_GLAp, attention_GLCAp])
        conc_GGCp = concatenate([avg_pool_Gp, max_pool_Gp, avg_pool_GGp, max_pool_GGp, avg_pool_GGCp, max_pool_GGCp, attention_GGAp, attention_GGCAp])        
...
# #         ==================================================================================================
        model_avg = Model(inputs=[inp, inp_post], outputs=[out_LL, out_LG, out_GL, out_GG, out_LLp, out_LGp, out_GLp, out_GGp, out_avg, out_avgp, out_last])
...
#         y_val = np.array(dataset_val.labels)
        ...
        hist = model.fit([x_pre, x_post], [y,y,y,y,
                                 y,y,y,y,
                                 y,y,y], 
