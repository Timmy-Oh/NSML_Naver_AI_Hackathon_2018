...
def bind_model(model, config):
    ...
    def infer(raw_data, **kwargs):
        ...
        point = model.predict(preprocessed_data)[2]
        point[point>10.] = 10.
        point[point<1.] = 1.
...
if __name__ == '__main__':
    ...
    def get_model(config):
        ...
        block1 = Conv1D(filter_nr, kernel_size=filter_size, padding='same', activation='linear', 
                    kernel_regularizer=conv_kern_reg, bias_regularizer=conv_bias_reg)(emb)
...
#         fc1 = Dense(50, activation='relu')(conc)
        ...
        rnn_out = Dense(1, activation='sigmoid')(fc1)
        
#         model = Model(inputs=inp, outputs=outp)
#         model.compile(loss='mean_squared_error', optimizer='adam', metrics=['mean_squared_error', 'accuracy'])
        
# #         ==================================================================================================
        ens_out = average([rnn_out, dpcnn_out])
        model = Model(inputs=inp, outputs=[rnn_out, dpcnn_out, ens_out])
        model.compile(loss='binary_crossentropy', optimizer='adam', loss_weights=[1.,1., 1.], metrics=['binary_crossentropy', 'accuracy'])
...
#                              batch_size=config.batch_size, callbacks=[nsml_callback], epochs=config.epochs, verbose=2)
    ...
    # [(0.0, 9.045), (0.0, 5.91), ... ]
        ...
        with open(os.path.join('../sample_data/kin/', 'train/train_data'), 'rt', encoding='utf-8') as f:
            reviews = f.readlines()
